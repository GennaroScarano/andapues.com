var typed = new Typed("#typed", {
  ...
  let showFigure = gsap.fromTo("figure, h3, button", {yPercent: -100}, {yPercent: 0}).progress(1);
  
  ScrollTrigger.create({
    start: 0,
    end: () => ScrollTrigger.maxScroll(window) - 2, // just above the bottom of the page
    onUpdate: self => {
      self.direction === 1 ? showFigure.reverse() : showFigure.play();
    },
    onLeave: () => showFigure.play() // we're at the bottom of the page
  });
  ...
  //stars
  scene3.fromTo("#stars, #stars1, #stars2, #stars3", { opacity: 0 }, { opacity: 0.5, y: -500 }, 0);
  
  // Scroll Back text
  scene3.fromTo("#arrow2", { opacity: 0 }, { opacity: 0.7, y: -810 }, 0.25);
  scene3.fromTo("#text2", { opacity: 0 }, { opacity: 0.7, y: -810 }, 0.3);
  ...
  //animate button
  scene3.fromTo(".btn_works", { opacity: 0 }, { opacity: 0.7, y: -200 }, 0.3);
  scene3.fromTo("#resume", { display: 'none' }, { display: 'block', y: -200 }, 0.3);
  scene3.fromTo("footer", { opacity: 0, y: -100 }, { opacity: 0.7, y: 0 }, 0.3);
  scene3.fromTo("#marketing", { display: 'none' }, { display: 'block', y: 0 }, 0.3);
  
  //animate ufo
  scene3.fromTo("#scene-space-social", { opacity: 0 }, { opacity: 1, y: -170 }, 0.3);
  ...
  fsEnter.addEventListener('click', function (e) {
    ...
    } else
    {
      ...
      fsEnter.innerHTML = "Go Fullscreen for Exciting Experience";
    }
  });